  ...
  return (
    <div className="space-y-4 pb-12 animate-fade-in">
      ...
      <div>
        ...
        <div className="space-y-1.5">
          ...
          <button
            ...
            className="w-full flex items-center space-x-3 bg-cyan-500 p-3.5 rounded-xl text-left hover:bg-cyan-400 active:scale-[0.98] transition-all"
          >
            <div className="w-10 h-10 rounded-lg bg-tactical-950/15 text-tactical-950 flex items-center justify-center font-bold">
              <ScanLine className="w-5 h-5 stroke-[2.5]" />
            </div>
            <div>
              <div className="text-xs font-black text-tactical-950 tracking-wide">NEW SCREENING</div>
              <div className="text-[10px] text-tactical-950/70 font-mono">Start a field scan</div>
            </div>
          </button>

          {/* Action 2: Live Sample Camera */}
          <button
            onClick={() => {
              triggerHapticTap();
              setIsCameraOpen(true);
            }}
            className="w-full flex items-center space-x-3 bg-slate-900/45 p-3 rounded-xl text-left hover:bg-slate-900/70 active:scale-[0.98] transition-all"
          >
            <div className="w-10 h-10 rounded-lg bg-slate-800 text-cyan-400 flex items-center justify-center font-bold">
              <Camera className="w-5 h-5" />
            </div>
            <div>
              <div className="text-xs font-bold text-white tracking-wide">LIVE SAMPLE CAMERA</div>
              <div className="text-[10px] text-slate-400 font-mono">Capture field sample evidence</div>
            </div>
          </button>

          {/* Action 3: Drug Heatmap */}
          <button
            onClick={() => {
              triggerHapticTap();
              onNavigate('heatmap');
          ...
          <button
            ...
            className="w-full flex items-center space-x-3 bg-slate-900/45 p-3 rounded-xl text-left hover:bg-slate-900/70 active:scale-[0.98] transition-all"
          >
            <div className="w-10 h-10 rounded-lg bg-slate-800 text-emerald-400 flex items-center justify-center font-bold">
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-slate-900/35 rounded-xl p-2.5">
        ...
        <div className="space-y-2">
          {screenings.slice(0, 1).map((sc) => (
            <div
              ...
              className="bg-slate-900/45 rounded-xl p-3 hover:border-slate-700 active:scale-[0.99] transition-all cursor-pointer flex items-center justify-between"
            >
      ...
      {isCameraOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-end sm:items-center justify-center p-3">
          <div className="w-full max-w-md bg-tactical-950 border border-slate-700/70 rounded-2xl p-3 shadow-2xl">
            <div className="flex items-center justify-between mb-3">
              <div>
                <div className="text-sm font-bold text-white">LIVE SAMPLE CAMERA</div>
                <div className="text-[10px] text-slate-400 font-mono mt-0.5">Capture sample / packaging evidence</div>
              </div>
              <button
                onClick={() => setIsCameraOpen(false)}
                className="text-slate-400 hover:text-white px-2 py-1 text-lg leading-none"
            ...
            <CameraViewfinder
              label="Align the live sample or packaging inside the frame"
              overlayType="substance"
              onCapture={() => setIsCameraOpen(false)}
            />
          </div>
        </div>
      )}
